---
name: duoduo-caldav-calendar-write
description: >
  Reinforced iCloud Calendar writer via CalDAV for this macOS sandbox. Use it whenever a task must
  CREATE, UPDATE, or DELETE events in 多多's iCloud calendars from this environment. Encodes the
  hard-won fixes: datetime normalization, proxy-off direct connect with 5xx retry, post-write readback
  verification, scoped deletes, 多多's original calendar color/system usage with conflict handling, and
  a 1-hour default block when the source has no explicit time. The only working bridge now that EventKit
  is blocked by the sandbox TCC.
agent_created: true
---
